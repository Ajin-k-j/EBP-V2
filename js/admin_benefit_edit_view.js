...
document
  ...
  .setAttribute("href", `delete_benefit_form.html?id=${id}`);
//setting link for add benefit button
document
  .getElementById("addBenefit")
  .setAttribute("href", `admin_add_benefit.html?id=${id}`);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $(".sideHeadingBox:eq(0)").attr(
    "href",
    `admin_benefit_edit_view.html?id=${categoryOtherData[0].id}`
  );
  $(".sideHeadingBox:eq(1)").html(categoryOtherData[1].name);
  $(".sideHeadingBox:eq(1)").attr(
    "href",
    `admin_benefit_edit_view.html?id=${categoryOtherData[1].id}`
  );
